        ...
        return new Promise(function(resolve, reject){
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState == XMLHttpRequest.DONE) {
                    ...
                    var buffer = []
                    answer.map(function( element, i) {
                        buffer.push({number:i+1, goodsCategories2: element.subcategory_2, goodsCategories3: element.subcategory_3, goodsType: element.name, amountOnWarehouse: element.amount, cost: parseFloat(element.price), weight: element.weight})
                        buffer[i].id = Api.getId()
                        buffer[i].code = element.code;
